...
- **GTNH 2.8 vs 2.9** — two things moved, and `module_api.lua` holds both. (1) 2.9 replaced the mining module's positional `setParameters(index, 0, value)` with a named `setParameter(key, value)`, and the two forms are disjoint. (2) 2.9 **beta 3** renamed the drone tier marker: `Mining Drone MK-IX (UHV)` became `Mining Drone Mk-IX (UHV)`. The second one matters more than it looks — labels are how drones are resolved in the ME network, so the wrong spelling counts **0 of every tier** and nothing ever dispatches, which reads as an empty network rather than a misconfiguration. The two changes have different boundaries, so `gtVersion` offers three values (`2.9`, `2.9-pre-b3`, `2.8`) for two parameter APIs, and `module_api.lua` maps version to dialect rather than conflating them.
- **Which version is a choice, not a probe** — set `gtVersion` on the editor's COMPATIBILITY page (`E`); it persists to `user_config.lua` like any other setting. It has to be told rather than probed, because an item name has nothing to probe for. `detect()` still runs at boot to *check* the answer against the hardware and warn on disagreement. The hw node gets the drone spelling pushed to it with `DRILL_PAR`, since it holds no config of its own. Nothing else on the miner path changed — `setWorkAllowed`, `isMachineActive`, `setInterfaceConfiguration` and the transposer are identical on both.
