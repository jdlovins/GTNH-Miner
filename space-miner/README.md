...
| `module_api.lua` | broker, job node | The GTNH 2.8 / 2.9 differences, in one place: the mining-module parameter API and the drone tier marker. Set by `gtVersion`. |
...
- **GTNH 2.8 vs 2.9** — the drone label changed **twice**, and `module_api.lua` holds every version difference. (1) 2.9 replaced the mining module's positional `setParameters(index, 0, value)` with a named `setParameter(key, value)`, and the two forms are disjoint. (2) 2.9 added the voltage to the drone's name: 2.8's `Mining Drone MK-IX` became `Mining Drone MK-IX (UHV)`. (3) 2.9 **beta 3** then lowercased the tier marker, to `Mining Drone Mk-IX (UHV)`. The label ones matter more than they look — labels are how drones are resolved in the ME network, so either half wrong counts **0 of every tier** and nothing ever dispatches, which reads as an empty network rather than a misconfiguration. The changes have different boundaries, so `gtVersion` offers three values (`2.9`, `2.9-pre-b3`, `2.8`) for two parameter APIs, and `module_api.lua` maps version to dialect and to label form separately rather than conflating them. Labels are *built* from a roman tier and a voltage (`config.droneTiers`), never rewritten from one spelling into another — dropping ` (UHV)` for 2.8 throws away what putting it back would need.
- **Dashboard controls** — the header row carries two buttons: `[ SETTINGS ]` opens the editor (the same quiesce countdown as pressing `E`, and clicking it again cancels the countdown) and `[ PAUSE ]` / `[ RESUME ]` stops the broker *starting* new jobs. Both have key equivalents, `E` and `P`, because a tier 1 screen cannot report a click at all — the buttons are the second way in, never the only one. Each key can be switched off on the editor's INTERFACE page (`hotkeyEditor`, `hotkeyPause`) for a screen other people walk past; the buttons are never gated on those, and the broker warns at boot if the editor key is off, since a screen that cannot be clicked has no way back in. Pause never interrupts a module already mining: it holds a drone and a kit, and cancelling the run wastes both, so the array drains to idle and stays there. It is session state and resets on restart, so a broker that comes back up always dispatches.
- **Which version is a choice, not a probe** — set `gtVersion` on the editor's COMPATIBILITY page (`E`); it persists to `user_config.lua` like any other setting. It has to be told rather than probed, because an item name has nothing to probe for. `detect()` still runs at boot to *check* the answer against the hardware and warn on disagreement. The hw node gets the drone label form — marker case and whether the voltage is in the name — pushed to it with `DRILL_PAR`, since it holds no config of its own. Nothing else on the miner path changed — `setWorkAllowed`, `isMachineActive`, `setInterfaceConfiguration` and the transposer are identical on both.
- **Drone counts are declared, not detected** — set them on the editor's HARDWARE page (`droneStock.<tier>`); nothing dispatches until you do. `hw_telem` can only see the ME network, and a drone in a module's input bus is not in it, so measuring a fixed pool of durable items through that sensor left the broker forever reconstructing the difference from dispatch timestamps — and getting it wrong in four distinct ways, each of which showed up as a module running a weaker drone than the fleet had free. Tips and rods are consumed and are still measured. Telemetry still reports drones, shown on the hardware panel as an audit against what you declared, so a fleet that has drifted says so instead of quietly mis-dispatching.
- **On GTNH 2.8, parallel and cycle are not settable from code** — they live in each module's own GUI. Set every module to its tier maximum, because dispatch assumes `moduleTiers[tier].maxParallels`; a GUI holding less still mines, but the computation draw and ETA readouts will be wrong. The broker prints one warning at boot when it is set to 2.8.
