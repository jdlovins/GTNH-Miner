...
- **GTNH 2.8 vs 2.9** — two things moved, and `module_api.lua` holds both. (1) 2.9 replaced the mining module's positional `setParameters(index, 0, value)` with a named `setParameter(key, value)`, and the two forms are disjoint. (2) 2.9 renamed the drone tier marker: `Mining Drone MK-IX (UHV)` became `Mining Drone Mk-IX (UHV)`. The second one matters more than it looks — labels are how drones are resolved in the ME network, so the wrong spelling counts **0 of every tier** and nothing ever dispatches, which reads as an empty network rather than a misconfiguration.
